...
try {
    $stmt = $pdo->prepare("SELECT * FROM pacientes ORDER BY nombre ASC");
    $stmt->execute();
        ...
        <table class="tabla-pacientes" id="tablaPacientes">
            ...
            <tbody>
                <?php foreach ($pacientes as $paciente): ?>
                    <tr data-paciente-id="<?php echo htmlspecialchars($paciente['id']); ?>" tabindex="0">
                        <td class="col-foto"><img src="images/avatars/<?php echo htmlspecialchars($paciente['foto'] ?: 'default_avatar.png'); ?>" alt="Foto de perfil"></td>
                        <td><input type="text" value="<?php echo htmlspecialchars($paciente['nombre']); ?>" name="nombre"></td>
                        <td><input type="text" value="<?php echo htmlspecialchars($paciente['apellido']); ?>" name="apellido"></td>
                        <td><input type="text" value="<?php echo htmlspecialchars($paciente['cedula']); ?>" name="cedula"></td>
                        <td><input type="email" value="<?php echo htmlspecialchars($paciente['email']); ?>" name="email"></td>
                        <td><input type="tel" value="<?php echo htmlspecialchars($paciente['whatsapp']); ?>" name="whatsapp"></td>
                        <td><input type="date" value="<?php echo htmlspecialchars($paciente['ultima_consulta']); ?>" name="ultima_consulta"></td>
                        <td class="col-acciones">
                            <div class="acciones-celda">
                                <!-- BOTÓN "VER PERFIL" AÑADIDO -->
                                <a href="index.php?page=paciente_perfil&id=<?php echo htmlspecialchars($paciente['id']); ?>" class="btn-accion btn-ver" title="Ver perfil completo">
